...
pub fn init(pack_file: std.fs.File, index_file: std.fs.File) !Odb {
...
pub fn readObject(odb: *Odb, allocator: mem.Allocator) !Object {
    ...
    var base_header = try EntryHeader.read(odb.pack_file.reader());
    ...
    var base_data = try odb.readObjectRaw(allocator, base_header.uncompressedLength());
    errdefer allocator.free(base_data);
    while (delta_offsets.popOrNull()) |delta_offset| {
        try odb.pack_file.seekTo(delta_offset);
        const delta_header = try EntryHeader.read(odb.pack_file.reader());
        var delta_data = try odb.readObjectRaw(allocator, delta_header.uncompressedLength());
...
pub fn readObjectRaw(odb: *Odb, allocator: mem.Allocator, size: usize) ![]u8 {
    var buffered_reader = std.io.bufferedReader(odb.pack_file.reader());
...
pub fn seekOid(odb: *Odb, oid: Oid) !void {
    const key = oid[0];
    ...
    const found_index = while (start_index < end_index) {
        ...
        try odb.index_file.seekTo(IndexHeader.size + mid_index * git.object_name_length);
        const mid_oid = try odb.index_file.reader().readBytesNoEof(git.object_name_length);
